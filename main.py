import json
import re
from js import Response, fetch as js_fetch

# ==========================================
# CẤU HÌNH BẢNG GIÁ & PROMPT
# ==========================================
...
async def supabase_request(env, endpoint, method="GET", body=None, extra_headers=None):
    ...
    return await js_fetch(url, req_options)
...
async def send_email_alert(env, subject, content):
    ...
    await js_fetch(url, {"method": "POST", "headers": headers, "body": json.dumps(body)})
...
async def call_gemini_ai(env, user_id, user_text):
    ...
    res = await js_fetch(url, {"method": "POST", "headers": {"Content-Type": "application/json"}, "body": json.dumps({"contents": contents})})
    data = await res.json()
    try:
...
async def send_fb_message(env, recipient_id, text):
    ...
    for chunk in split_message(text):
        await js_fetch(url, {"method": "POST", "headers": {"Content-Type": "application/json"}, "body": json.dumps({"recipient": {"id": recipient_id}, "message": {"text": chunk}})})

# ==========================================
# ENTRY POINT CHÍNH (BẮT BUỘC DÙNG TÊN "fetch")
# ==========================================
